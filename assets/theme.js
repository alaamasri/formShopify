...
slate.utils = {
  ...
  removeCollectionViewAllOption: function(forceRemove,callback){
    var _this = this;
    var collectionSettings = this.getCollectionSettings();

    if(!collectionSettings){
      collectionSettings = {}
    }
    
    if(!collectionSettings.viewAllClicked){
      if(callback){
        callback();
      }
      return;
    ...
    if(window.location.pathname.indexOf('/collection') !== 0 || forceRemove){
      CartJS.setAttribute("pagination",null,{
        "success": function(data, textStatus, jqXHR) {
          _this.saveCollectionSettings('viewAllClicked',false);
          if(callback){
            callback(null,true);
          }
        },
  ...
  saveCollectionSettings: function(key, value){
    var _settings = this.getCollectionSettings() || {};
    _settings[key] = value;
    _settings = JSON.stringify(_settings);
  	localStorage.setItem("collectionSettings", _settings);
  },
  
  getCollectionSettings: function(){
  	var _settings = localStorage.getItem("collectionSettings");
    if(_settings){
      try{
...
slate.Variants = (function() {
  ...
  Variants.prototype = _.assignIn({}, Variants.prototype, {
    ...
    _onSelectChange: function() {
	  ...
	  window.dispatchEvent(new CustomEvent('variantChange', {detail: {variant: variant}}));
      ...
      //Alaa code: Embroidery varient image
      this.vImg = document.getElementsByClassName('variant-img');
      this.selectedVarient= this.currentVariant.title.split(" / ", 1).toString().split('_',1);
      this.selectedVarientPart2= this.currentVariant.title.split(" / ", 1).toString().split('_');
      
             if( this.selectedVarient == 'img' || this.selectedVarientPart2[1].indexOf('-') > -1 )
             {this.selectedVarient=this.selectedVarientPart2[1]  } else{this.selectedVarient=this.selectedVarient[0]}
      ...
      if(this.vImg.length){
    		...
    		this.vImg[i].style.display = 'none';
        	}
          }
          
          if(document.getElementById("varImg_"+this.selectedVarient) != null){
        document.getElementById("varImg_"+this.selectedVarient).style.display = 'block';
          }else{
          document.getElementById("defVarImg").style.display = 'block';
          }
...
theme.Product = (function() {
  ...
  Product.prototype = _.assignIn({}, Product.prototype, {
    ...
    _filterThumbnails: function(evt){
	  this._destroyThumbnailSlider();
      var variant = evt.variant;
      var slideElement = null;
      
      if(variant && variant.featured_image && variant.featured_image.alt){
          //hide all first
          $('[data-thumbnail-color]').hide();
        
          //show thumbnails for the selected variant
          var selected_color = variant.featured_image.alt;
          selected_color = selected_color.replace(" ","-");
          var thumbnail_selector = '[data-thumbnail-color="'+ selected_color +'"]';
        
          slideElement = '.slide--' + selected_color;
      ...
      this._initThumbnailSlider({slideElement : slideElement });
    },
    
    _initBreakpoints: function() {
      var self = this;
        // initialize thumbnail slider if more than three thumbnails
    ...
    _initVariants: function() {
      ...
      this._filterThumbnails({variant : this.variants.currentVariant});
      ...
      this.$container.on(
        'variantChange' + this.settings.namespace,
        this._filterThumbnails.bind(this)
      );
      this.$container.on(
    ...
    _initImageSwitch: function() {
      ...
      $(this.selectors.productThumbImages)
        .on('click', function(evt) {
	      ...
	      window.dispatchEvent(new CustomEvent('productThumbImagesClicked', {detail: {imageId: imageId}}));
        })
    ...
    _addItemToCart: function(data) {
      ...
      var customEmbroideryItems = null;
      var guid = null;
      var _this = this;
      var form_data_obj = { };
      ...
      function _addOriginalItem(callback){
		var _cartItems = [];
        //Add the orignal product items to the cart items
        var _productFormDataArray = $(data).serializeArray();
        var _productFormDataObj = {};
        for(var e=0; _productFormDataArray && e < _productFormDataArray.length; e++){
            _productFormDataObj[_productFormDataArray[e].name] = _productFormDataArray[e].value;
        }

        //if guid then add it with _ prefix so we can hide it in cart page
        if(guid){
          if(!_productFormDataObj['properties']){
            _productFormDataObj['properties'] = {};
          }

          _productFormDataObj['properties']['_guid'] = guid;
        }

        _cartItems.push(_productFormDataObj);
        ...
        $.post(params).done(
            function(item) {
              ...
              //it used to return one item, but now we add array of items, as we did in embroidery
              if(item && item.items && item.items.length > 0){
                item = item.items[0].id == _productFormDataObj['id'] ? item.items[0] : item.items[1];
              }
              this._hideErrorMessage();
              this._setupCartPopup(item);
              window.dispatchEvent(new CustomEvent('addedToCart', {detail: {item: item}}));
            }.bind(_this)
          )
          .fail(
            function(response) {
      ...
      if($(this.selectors.productForm, this.$container).attr('data-has-embroidery')){
        if($(this.selectors.productForm, this.$container).attr('data-embroidery-items')){
			customEmbroideryItems = $(this.selectors.productForm, this.$container).attr('data-embroidery-items');
            try{
              customEmbroideryItems = JSON.parse(customEmbroideryItems);
            }
            catch(e){
            //do nothing
            }
        ...
        var embroidery_variant_id = $(this.selectors.productForm, this.$container).attr('data-embroidery-variant_id');
        
        if(customEmbroideryItems){
          var _embroideryProperties = {};
          var formData = new FormData();
		  var _quantity = form_data_obj['quantity'] ? parseInt(form_data_obj['quantity']) : 1;
		  var _color = form_data_obj['Color'];
		  var _size = form_data_obj['Size'];
          
		  guid = slate.utils.uuidv4();
		  formData.append('properties[_guid]', guid);
          formData.append('id', embroidery_variant_id);
          formData.append('quantity', _quantity);
          if(_color){
            formData.append('properties[_color]', _color);
          }
          
          if(_size){
            formData.append('properties[_size]', _size);
          }
          
          for(var i=0; i < customEmbroideryItems.length; i++){
            _embroideryProperties[customEmbroideryItems[i].key] = customEmbroideryItems[i].value;
            if(customEmbroideryItems[i].type === 'file'){
              formData.append('properties[' + customEmbroideryItems[i].key + ']', window.embroidery_context.uploadedLogoImage);
            }else{
              formData.append('properties[' + customEmbroideryItems[i].key + ']', customEmbroideryItems[i].value);              
            }

          }
          
		  formData.append('properties[_isEmbroidery]', true);              
	  ...
	  //#handle custom embroidery properties
    },
    ...
    _updateCartPopupContent: function(item) {
      ...
      $.getJSON('/cart.js').then(
        function(cart) {
          var cart_count_wihout_embroidery_items = calculateCartWithoutEmbroidery(cart.items);
          this._setCartQuantity(cart_count_wihout_embroidery_items);
          this._setCartCountBubble(cart_count_wihout_embroidery_items);
          this._showCartPopup();
        }.bind(this)
      );
      
      function calculateCartWithoutEmbroidery(cartItems){
        var count = 0;
        for(var i = 0;i < cartItems.length ;i++){
          if(cartItems[i].properties){
            if(!cartItems[i].properties["_isEmbroidery"]){
    ...
    _initThumbnailSlider: function(slideOptions) {
	  var self = this;
      if(!slideOptions){
        slideOptions= {};
      }
          ...
          },
 {
      ...
      if(slideOptions.slideElement){
      	options['slide']= slideOptions.slideElement;
      }
